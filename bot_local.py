...
def send_about(message):
    ...
    with open('template_text/about.txt', mode='r', encoding='utf-8') as f:
        ...
        about = temp.substitute(
            STUDENT_NAME = 'NAMIRA SUNIAPRITA',
            BATCH_ACADEMY = 'Sparta Day',
            GITHUB_REPO_LINK = 'https://github.com/namiraprita')
...
#         # TO DO: find the range date
        dff_campaign = df[df["campaign_id"] == selected_campaign_id]
        
        start_date = dff_campaign['reporting_date'].min().strftime(format= '%d %b %Y')
        end_date = dff_campaign['reporting_date'].max().strftime(format='%d %b %Y')
        
#         # TO DO: perform calculation
        total_spent = int(sum(dff_campaign["spent"]))
        total_conversion = int(sum(dff_campaign["total_conversion"]))
...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        dff_campaign = df[df["campaign_id"] == selected_campaign_id]
        df_plot = dff_campaign.pivot_table(index='age', values = ['spent', 'approved_conversion'], aggfunc='sum' )
